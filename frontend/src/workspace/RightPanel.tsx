...
export function RightPanel({ results, parsedSource, tone, generating, selectedFormats, notice, onDismissNotice, status }: Props) {
  ...
  const handleRegenerate = async () => {
    if (!currentTab || !activeResult || !refinement.trim() || regenerating) return
  ...
  if (generating && tabs.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        <CanvasHeader
          title="Output canvas"
          status={`Drafting ${selectedFormats.length} ${selectedFormats.length === 1 ? 'format' : 'formats'}`}
        />
