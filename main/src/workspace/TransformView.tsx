  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-[1760px] px-4 pb-14 sm:px-6 lg:px-10 2xl:px-14">
        ...
        {/* Page header: title left, live session panel right */}
        <header className="grid gap-8 border-b border-ink/10 pb-8 pt-10 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:gap-12 lg:pb-10 lg:pt-12">
        ...
        {/* Two white sheets on the canvas: the control desk and the output canvas */}
        <div className="mt-8 grid gap-6 lg:mt-10 lg:grid-cols-[minmax(420px,5fr)_minmax(0,7fr)] lg:gap-8 2xl:gap-10">
          <aside
            aria-label="Source and configuration"
            className="sk-elevated flex flex-col rounded-2xl border border-line bg-white lg:h-[max(680px,calc(100dvh-7rem))] lg:overflow-hidden"
          >
          ...
          <section
            ...
            className="sk-elevated relative flex min-h-[640px] flex-col overflow-hidden rounded-2xl border border-ink bg-white lg:h-[max(680px,calc(100dvh-7rem))] lg:min-h-0"
          >
            <RightPanel
              runs={runs}
...
function SessionPanel({ drafts, lastMs, generating, engineLabel, engineReady, onOpenHistory }: SessionPanelProps) {
  ...
  return (
    <div
      aria-label="Session"
      className="sk-elevated flex w-full items-stretch overflow-hidden rounded-2xl border border-line bg-white lg:w-auto"
    >
      <dl className="flex min-w-0 flex-1 divide-x divide-line">
        <div className="min-w-0 flex-1 px-4 py-3.5 sm:px-5 lg:min-w-[120px]">
          <dt className="text-[12px] text-ink-mute">Engine</dt>
          <dd className="mt-1.5 flex h-6 items-center gap-2 text-[14px] font-semibold text-ink" aria-live="polite">
        ...
        <div className="min-w-0 flex-1 px-4 py-3.5 sm:px-5 lg:min-w-[120px]">
          <dt className="text-[12px] text-ink-mute">Drafts</dt>
          <dd className="mt-1.5 flex h-6 items-center font-display text-[24px] leading-none text-ink tabular-nums">{drafts}</dd>
        </div>
        <div className="hidden min-w-0 flex-1 px-4 py-3.5 sm:block sm:px-5 lg:min-w-[112px]">
          <dt className="text-[12px] text-ink-mute">Last run</dt>
          <dd className="mt-1.5 flex h-6 items-center font-display text-[24px] leading-none text-ink tabular-nums">
            {lastMs === null ? <span className="font-sans text-[14px] font-medium text-ink-mute">No run yet</span> : `${(lastMs / 1000).toFixed(1)}s`}
          </dd>
        </div>
      </dl>

      {onOpenHistory && (
        <div className="flex shrink-0 items-center border-l border-line bg-paper-deep p-2">
          <button
            type="button"
            onClick={onOpenHistory}
            className="group flex h-full items-center gap-2 rounded-xl bg-ink px-4 text-[13.5px] font-medium text-paper transition-[transform,background-color] duration-200 hover:bg-ink-soft focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-matcha active:scale-[0.98]"
          >
            History
